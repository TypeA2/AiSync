...
const ffmpeg_path = which.sync("ffmpeg", { nothrow: false });
const ffprobe_path = which.sync("ffprobe", { nothrow: false });
...
export async function extract_stream(file: string, id: number, target: string, copy: boolean = true): Promise<void> {
    return extract_streams(file, [id], target, copy);
}

export async function extract_streams(file: string, ids: number[], target: string, copy: boolean = true): Promise<void> {
    const argv = [
        "-v", "quiet",
        "-i", file,
    ];
